...
Claude ultracode;
  ...
  - Use the best worker, at the most economical rate, to complete the task (efficiency
    optimization, with some quality prioritization as pipelines are built out)
...
2. The "Providers & limits" panel does the rest for you: a provider that is missing shows Install,
   one that is installed but signed out shows Sign in; both open a real terminal window (browser
   logins need one), then press Refresh. Subscription CLIs are workers (delegate targets); their
   models appear in the worker picker.

Use Chrome or Edge for the microphone button (Web Speech API). The page is served from 127.0.0.1
only.

Prefer a terminal? See "Quickstart from a terminal" at the end.
...
3. Watch worker tasks in the Fleet panel (click a card for spec, actions, diff stat, worker
   report). Approve or deny permission prompts inline when not in auto-approve mode.
4. The microphone button or Ctrl+M dictates into the composer; edit, then Enter to send.
...
Command line:
  ...
  conductor limits --refresh
  conductor scores
  conductor smoke --models codex:gpt-5.6-luna:low     (seeds the scorecard; spends budget)
  conductor review
  conductor share
...
subagents (haiku swarm / sonnet worker / adversarial reviewer) are available through the built-in
Agent tool, and limits are checked before big batches.
...
Root CLAUDE.md (Claude) / AGENTS.md (Codex) are read natively. Per-folder CONTEXT.md notes are
injected into worker specs for the paths in scope, so large projects stay modular. The conductor is
instructed to create or update CONTEXT.md when it adds modules.
...
conductor and you can add ideas (log_improvement tool, sidebar "Open log"). "Run review" opens a
conductor session on this repository with the log as input; it delegates fixes, runs "npm test",
and marks entries resolved. Or run "conductor review" headless.
...
latest commit and runs "npm install" when dependencies changed; restart Conductor afterwards. It
refuses while you have uncommitted or unpushed local changes, so commit and push from GitHub
Desktop first. Your state (settings, keys, scorecard, task journal) lives in ~/.conductor2 on each
machine and is never part of the repo.


FEEDBACK
--------

"conductor feedback" writes Conductor-feedback-<date>.json to your Desktop (versions, which
providers are set up, limit windows, the improvement log: errors the workbench caught, ideas you
logged; and the scorecard) with your home path, user name, e-mail addresses and anything key-shaped
redacted. It then opens the project's issue page so you can attach the file. Nothing is sent
automatically.
...
Then run share/install.cmd (Windows) or share/install.sh, and log in to your own accounts (nothing
in the repo carries anyone's keys or logins: they live in ~/.conductor2/config.json and the vendor
CLIs' own login state). "conductor share" still zips the committed files (what git tracks, so never
local state) to your Desktop for offline hand-offs.
...
1. Run the exe, same as above. (Conductor.cmd does the same in a console; rebuild the exe with
...
3. One-time logins (in a terminal, not in the app):
   ...
   Claude subscription (the conductor)
       claude auth login
       (if "claude" is missing: npm i -g @anthropic-ai/claude-code)

   ChatGPT subscription (Astra worker)
       npm i -g @openai/codex
       codex login

   Google AI Pro/Ultra (Gemini 3.x via the Antigravity CLI)
       irm https://antigravity.google/cli/install.ps1 | iex
       then run "agy" once and sign in

   SuperGrok / X Premium+ (Grok CLI)
