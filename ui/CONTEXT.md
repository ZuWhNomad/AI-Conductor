...
**How to test.** `test/ui-settings.test.mjs` exercises settings rendering and saving with a DOM stub.
`npm test` also covers serving (`test/server/server.test.mjs`: `/` and `/app.js`).
