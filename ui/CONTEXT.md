...
**How to test.** `npm test` covers serving (`test/server/server.test.mjs`: `/` and `/app.js`) and browser regressions
(`test/ui/layout.test.mjs`: responsive geometry, quit icon, budget freshness and emphasis). Browser cases use a local
Edge/Chrome installation or `CONDUCTOR_TEST_BROWSER`; they explicitly skip when no browser is available.
